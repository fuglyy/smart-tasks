...
function addTask() {
    ...
    if (Notification.permission === 'granted') {
        sendNotification('New Task Added', `"${text}" has been added to your task list.`);
    }
}
...
function setupNotificationButton() {
    ...
    notificationBtn.addEventListener('click', async () => {
        ...
        // Если разрешение уже есть, переключаем состояние подписки
            ...
            const isCurrentlySubscribed = localStorage.getItem('notificationsEnabled') === 'true';
...
function scheduleNotifications() {
    ...
    notificationInterval = setInterval(() => {
        console.log('Проверка незавершенных задач:', new Date().toLocaleTimeString());
        ...
        if (incompleteTasks.length > 0 && Notification.permission === 'granted') {
            console.log('Отправка уведомления о', incompleteTasks.length, 'незавершенных задачах');
            sendNotification(
...
function scheduleNotifications() {
    ...
    notificationInterval = setInterval(() => {
        console.log('Checking for incomplete tasks at:', new Date().toLocaleTimeString());
        const incompleteTasks = tasks.filter(task => !task.completed);
        
        if (incompleteTasks.length > 0 && Notification.permission === 'granted') {
            console.log('Sending notification for', incompleteTasks.length, 'incomplete tasks');
            sendNotification(
